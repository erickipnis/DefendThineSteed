...
public class TrollGeneticAlgorithm
{	
	...
	public static int[] steedsCaptured;
	...
	public static void SetUpGA()
	{
		...
		tp = new ThreshPop(chromLeng, popSize, "./GeneticAlgorithmData/steedBayes.txt");	 
		...
		steedsCaptured = new int[popSize];
	...
	{
		StreamWriter outStream = new StreamWriter ("./GeneticAlgorithmData/trollDistances.txt");
		
		for (int i = 0; i < phenos.Length; i++)
		{
			outStream.WriteLine("Geno: " + chroms[i].ToString() + ", SteedSeekDistance: " + phenos[i].ToString());
		}
	...
	public static int Fitness (float fitness, int capturedIndex)
	{
		return (int)(fitness + (2 * steedsCaptured[capturedIndex]) + Util.rand.Next(100));	// S/N = 1:1
	}
}
