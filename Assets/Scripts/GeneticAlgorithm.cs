...
{	
	//TODO: Create fitness counter that goes down by deltaTime * 0.5 each frame
	//		Assign each troll a steed distance (phenotype)
	...
	public static void SetUpGA()
	{
		...
		tp = new ThreshPop(chromLeng, popSize, "./GeneticAlgorithmData/trollData.txt");	 
		...
		phenotypeArray = new float[popSize];
		...
		while (! tp.AllCheckedOut())
		{
			...
			phenotypeArray[i] = Gen2Phen(chroms[i]);
			i++;
		}
	...
	public static void CheckInIndividual(float fitness, int index)
	{
		if (! tp.AllCheckedIn())
		{
			int fit = Fitness(fitness);
			tp.CheckIn(chroms[index], fit);
		}
	}
	...
	public static int Fitness (float fitness)
	{
		return (int)(fitness);	// S/N = 1:1
	}
}
